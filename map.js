...
mapManager.draw = function(ctx){//нарисовать карту в контексте
	...
	if(!mapManager.imgLoaded || !mapManager.jsonLoaded){
		setTimeout(function(){mapManager.draw(ctx);},100);
			...
			for(var i = 0; i < mapManager.tLayer.data.length; i++){// проойти по всей 
			//карте
				if(mapManager.tLayer.data[i] !==0){//если нет данных - 
				//пропускаем
					var tile = mapManager.getTile(mapManager.tLayer.data[i]);//получение блока 
					//по индексу
					//i  проходит линейно по массиву. xCount-длина по x
					var pX = (i%mapManager.xCount)*mapManager.tSize.x;//вычисляем x  
					//в пикселах
					var pY = Math.floor(i/mapManager.xCount)*mapManager.tSize.y
					//вычисляем  y
					//рисуем в контекст
...
var Scaven = {
	pos_x:480, 
	pos_y:480,//позиция объекта врага
}
var spriteManager = {
	atlas:null,
	image: new Image(),
...
var Player = Object.create(Geometry);
	Player.lifetime:100,
	Player.move_x:0,move_y:0,//направление движения
	Player.speed:1,// скорость игрока
	Player.draw: function(ctx){},//прорисовка игрока
	Player.update: function(){},//обновление в цикле
	Player.onTouchEntity:function(obj){},//обработка встречи с препятствием
	Player.onTouchMap:function(idx){},//обработка встречи с лавой
	Player.kill:function(){},//уничтожение объекта
	Player.attack: function(){}//атака
var Enemy = Object.create(Geometry);
	Enemy.lifetime:100,
	Enemy.move_x:0,move_y:0,//направление движения
 	Enemy.speed:1,// скорость игрока
	Enemy.draw: function(ctx){spriteManager.draw.Sprite(ctx, "left_stand", spriteManager.pos_x, spriteManager.pos_y);},//прорисовка игрока
 	Enemy.update: function(){},//обновление в цикле
 	Enemy.onTouchEntity:function(obj){},//обработка встречи с препятствием
 	Enemy.onTouchMap:function(idx){},//обработка встречи с лавой
 	Enemy.kill:function(){},//уничтожение объекта
 	Enemy.attack: function(){}//атака
